书生·浦语大模型训练营
---
在这次书生·浦语大模型实战营的学习中，我从零开始完成了一次完整的 LLM 开发实践。从搭建开发环境、配置 SSH 连接，到运行第一个 Hello World 程序、完成 LeetCode 算法题，再到利用 Hugging Face 平台下载与上传模型，我一步步深入掌握了实际开发中的常用工具链。

随着学习的推进，我尝试了更复杂的任务：结合 LlamaIndex 构建 RAG 应用，使用 XTuner 对 InternLM 模型进行微调，并最终通过 OpenCompass 完成了模型评测。整个过程不仅让我对书生·浦语系列模型（如 InternLM2.5）的强大能力有了更直观的认识，也让我系统性地掌握了从模型调用到定制优化、再到效果评估的完整技术流程。
